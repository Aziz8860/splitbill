...
async function uploadFileUrl(imageUrl) {
  ...
  formData.append(
    'system_prompt',
    `You are a receipt parsing assistant. Extract structured data from the receipt markdown.
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `LlamaParse upload API error (${response.status}): ${errorText}`
    );
  }
...
async function waitForJobCompletion(jobId, maxAttempts = 30, delayMs = 2000) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    console.log(
      `Job status (attempt ${attempt + 1}/${maxAttempts}):`,
      status.status
    );
    ...
    if (status.status === 'SUCCESS') {
      ...
      throw new Error(
        `LlamaParse job failed: ${status.error || 'Unknown error'}`
      );
    }
...
async function checkJobStatus(jobId) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `LlamaParse job status API error (${response.status}): ${errorText}`
    );
  }
...
async function getJobResult(jobId) {
  ...
  if (!response.ok) {
    const errorText = await response.text();
    throw new Error(
      `LlamaParse result API error (${response.status}): ${errorText}`
    );
  }
...
function formatReceiptData(data) {
  try {
    ...
    for (const field of possibleItemsFields) {
      if (
        jsonData[field] &&
        Array.isArray(jsonData[field]) &&
        jsonData[field].length > 0
      ) {
    ...
    const tax =
      parseFloat(jsonData.tax.amount || jsonData.tax.amount || 0) || 0;
    const subtotal =
      parseFloat(
        jsonData.subtotal || jsonData.sub_total || jsonData.subTotal || 0
      ) || 0;
    let totalAmount =
      parseFloat(
        jsonData.totalAmount || jsonData.total_amount || jsonData.total || 0
      ) || 0;
